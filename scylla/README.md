...
[![Maven Central](https://img.shields.io/maven-central/v/io.goodforgod/testcontainers-extensions-scylla.svg)](https://central.sonatype.com/artifact/io.goodforgod/testcontainers-extensions-scylla)
...
Testcontainers Scylla Extension with advanced testing capabilities.
...
ScyllaDB is a high-performance, distributed NoSQL database designed as a drop-in replacement for Apache Cassandra. 
It achieves low latency and high throughput by using a highly optimized, shard-per-core architecture written in C++.
...
```java
@TestcontainersScylla(mode = ContainerMode.PER_RUN,
        migration = @Migration(
...
`ScyllaConnection` is an abstraction with asserting data in database container and easily manipulate container connection settings.
You can inject connection via `@ConnectionScylla` as field or method argument or manually create it from container or manual settings.

```java
class ExampleTests {

    private static final ScyllaContainer<?> container = new ScyllaContainer<>();
    
    @Test
    void test() {
      container.start();
      ScyllaConnection connection = ScyllaConnection.forContainer(container);
...
You can migrate container via `@TestcontainersScylla#migration` annotation parameter or manually using `ScyllaConnection`.
...
`@TestcontainersScylla` - allow **automatically start container** with specified image in different modes without the need to configure it.
...
When you need to **manually configure container** with specific options, you can provide such container as instance that will be used by `@TestcontainersScylla`,
this can be done using `@ContainerScylla` annotation for container.
...
```java
@TestcontainersScylla(network = @Network(shared = true))
class ExampleTests {
...
```java
@TestcontainersScylla(network = @Network(alias = "${MY_ALIAS_ENV|my_default_alias}"))
class ExampleTests {
...
`ScyllaConnection` - can be injected to field or method parameter and used to communicate with running container via `@ConnectionScylla` annotation.
`ScyllaConnection` provides connection parameters, useful asserts, checks, etc. for easier testing.
...
`Isolation.Mode.PER_METHOD` reuses the same physical Scylla container but creates a separate generated keyspace for every test method. Field and method argument injection receive a `ScyllaConnection` configured for that generated keyspace.
...
In case you want to use some external Scylla instance that is running in CI or other place for tests (due to docker limitations or other),
you can use special *environment variables* and extension will use them to propagate connection and no Scylla containers will be running in such case.

Special environment variables:
- `EXTERNAL_TEST_SCYLLA_USERNAME` - Scylla instance username (optional).
- `EXTERNAL_TEST_SCYLLA_PASSWORD` - Scylla instance password (optional).
