...
class ScyllaConnectionImpl implements ScyllaConnection {
    ...
    private volatile ScriptScyllaMigrationEngine scyllaMigrationEngine;
    private volatile CognitorScyllaMigrationEngine cognitorMigrationEngine;

    ScyllaConnectionImpl(Params params, Params network) {
        this.params = params;
        this.network = network;
    }

    static ScyllaConnection forContainer(String host,
                                         int port,
        ...
        return new ScyllaConnectionImpl(params, network);
    }

    static ScyllaConnection forExternal(String host,
                                        int port,
                                        String datacenter,
                                        String keyspace,
                                        String username,
        ...
        return new ScyllaConnectionImpl(params, null);
    }
    ...
    @Override
    public @NotNull ScyllaMigrationEngine migrationEngine(Migration.@NotNull Engines engine) {
        if (engine == Migration.Engines.SCRIPTS) {
            if (scyllaMigrationEngine == null) {
                this.scyllaMigrationEngine = new ScriptScyllaMigrationEngine(this);
            }
            return this.scyllaMigrationEngine;
        } else if (engine == Migration.Engines.COGNITOR) {
            if (cognitorMigrationEngine == null) {
                this.cognitorMigrationEngine = new CognitorScyllaMigrationEngine(this);
            }
            return this.cognitorMigrationEngine;
        }
    ...
    private CqlSession sessionWithKeyspace(@NotNull String keyspaceName, CqlSessionBuilder sessionBuilder) {
        try (var session = sessionBuilder.build()) {
            ...
            throw new ScyllaConnectionException(e);
        }
    }
    ...
    private Optional<String> loadStringFromResources(String path) {
        try {
            try (var resource = ScyllaConnectionImpl.class.getClassLoader().getResourceAsStream(path)) {
                if (resource == null) {
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || getClass() != o.getClass())
            return false;
        ScyllaConnectionImpl that = (ScyllaConnectionImpl) o;
