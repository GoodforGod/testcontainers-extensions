...
@Internal
class RedpandaConnectionImpl implements RedpandaConnection {
    ...
    static final int REDPANDA_PORT = 9092;
    ...
    private static final Logger logger = LoggerFactory.getLogger(RedpandaConnection.class);
    ...
    RedpandaConnectionImpl(Properties properties, @Nullable Properties propertiesInNetwork) {
    ...
    static final class ConsumerImpl implements Consumer {
        ...
        ConsumerImpl(KafkaConsumer<byte[], byte[]> consumer, String clientId, Collection<TopicPartition> topicPartitions) {
            ...
            logger.trace("RedpandaConsumer topics {} assigning...", this.topics);
            ...
            logger.debug("RedpandaConsumer topics {} assigned", this.topics);

            logger.trace("RedpandaConsumer topics {} poll starting", this.topics);
            poll(Duration.ofMillis(50));
            this.executor.execute(this::launch);
            logger.debug("RedpandaConsumer topics {} poll started.", this.topics);
        }

        private void launch() {
            logger.info("RedpandaConsumer '{}' started consuming events from topics: {}", clientId, topics);
            while (isActive.get()) {
                try {
                    ...
                    logger.error("RedpandaConsumer '{}' for {} topics got unhandled exception", clientId, topics, e);
        ...
        private void poll(Duration maxPollTimeout) {
            ...
            if (!records.isEmpty()) {
                logger.info("RedpandaConsumer '{}' polled '{}' records from topics: {}", clientId, records.count(), topics);
            } else {
                logger.trace("RedpandaConsumer '{}' polled '{}' records from topics {}...", clientId, records.count(), topics);
            }
        ...
        public @NotNull List<ReceivedEvent> getReceivedAtLeast(int expectedEvents, @NotNull Duration timeout) {
            ...
            if (receivedEvents.size() == expectedEvents) {
                logger.debug("RedpandaConsumer '{}' received at least {} records from topics: {}",
        ...
        private List<ReceivedEvent> getReceivedEqualsInTime(int expected, @NotNull Duration timeToWait) {
            try {
                ...
                logger.debug("RedpandaConsumer '{}' received equals {} records in {} from topics: {}",
        ...
        void stop() {
            if (isActive.compareAndSet(true, false)) {
                logger.debug("Stopping RedpandaConsumer '{}' for {} topics...", clientId, topics);
                ...
                try {
                    ...
                    reset();
                    logger.info("Stopped RedpandaConsumer '{}' for {} topics took {}", clientId, topics,
    ...
    @Override
    public @NotNull RedpandaConnection withProperties(@NotNull Properties properties) {
        final Properties allProps = new Properties();
        allProps.putAll(params.properties());
        ...
        return new RedpandaConnectionClosableImpl(allProps, networkProperties);
    }
    ...
    public void send(@NotNull String topic, @NotNull List<Event> events) {
        if (isClosed) {
            throw new RedpandaConnectionException("Can't send cause was closed");
        }
        ...
        if (this.producer == null) {
            try {
                ...
                throw new RedpandaConnectionException("Can't create RedpandaProducer", e);
            }
        }
        ...
        for (Event event : events) {
            ...
            try {
                logger.trace("RedpandaProducer sending event: {}", event);
                var result = producer.send(new ProducerRecord<>(topic, null, key, event.value().asBytes(), headers))
                        .get(10, TimeUnit.SECONDS);
                logger.info(
                        "RedpandaProducer sent event to topic '{}' with offset '{}' with partition '{}' with timestamp '{}' event: {}",
                        topic, result.offset(), result.partition(), result.timestamp(), event);
            } catch (Exception e) {
                throw new RedpandaConnectionException("RedpandaProducer sent event failed: " + event, e);
            }
        }
    }
    ...
    public @NotNull Consumer subscribe(@NotNull Set<String> topics) {
        if (isClosed) {
            throw new RedpandaConnectionException("Can't subscribed cause was closed");
        }
        ...
        try {
            ...
            final ConsumerImpl topicConsumer = consumerByTopic.computeIfAbsent(consumerTopicKey, k -> {
                var kafkaConsumer = getConsumer(id, params.properties());
                return new ConsumerImpl(kafkaConsumer, id, topicPartition);
            });

            if (topicConsumer.isClosed()) {
                var newTopicConsumer = getConsumer(id, params.properties());
                ConsumerImpl activeConsumer = new ConsumerImpl(newTopicConsumer, id, topicPartition);
            ...
            throw new RedpandaConnectionException("Can't create RedpandaConsumer", e);
        }
    }
                        ...
                        .until(() -> {
                            try {
                                ...
                                if (e.getCause() instanceof TopicExistsException) {
                                    ...
                                    throw new RedpandaConnectionException("Redpanda Admin operation failed for topics: " + topics,
                                            e);
                                }
                            } catch (Exception e) {
                                throw new RedpandaConnectionException("Redpanda Admin operation failed for topics: " + topics, e);
                            }
                        });
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || getClass() != o.getClass())
            return false;
        RedpandaConnectionImpl that = (RedpandaConnectionImpl) o;
