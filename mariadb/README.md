...
**Gradle**
```groovy
testImplementation "io.goodforgod:testcontainers-extensions-mariadb:0.2.0"
```
...
class ExampleTests {
    ...
    @Test
    void test(@ContainerMariadbConnection JdbcConnection connectionInParam) {
...
Given engine is [Flyway](https://documentation.red-gate.com/fd/mariadb-184127600.html) and migration file named `V1__flyway.sql` is in resource folder on default path `db/migration`:
```sql
