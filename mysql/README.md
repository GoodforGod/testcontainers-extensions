...
**Gradle**
```groovy
testImplementation "io.goodforgod:testcontainers-extensions-mysql:0.2.0"
```
...
class ExampleTests {
    ...
    @Test
    void test(@ContainerMysqlConnection JdbcConnection connectionInParam) {
...
Given engine is [Flyway](https://documentation.red-gate.com/fd/mysql-184127601.html) and migration file named `V1__flyway.sql` is in resource folder on default path `db/migration`:
```sql
