...
@Internal
class ValkeyConnectionImpl implements ValkeyConnection {
    ...
    ValkeyConnectionImpl(Params params, Params network) {
        this.params = params;
    ...
    static ValkeyConnection forContainer(String host,
        ...
        return new ValkeyConnectionImpl(params, network);
    }
    ...
    private JedisConnection connection() {
        if (isClosed) {
            throw new IllegalStateException("ValkeyConnection was closed");
        }
        ...
        if (jedis == null) {
            try {
                ...
                try {
                    ...
                    throw new ValkeyConnectionException(e);
                }
            }
        }
    ...
    private JedisCommandsImpl getJedis() {
        var config = DefaultJedisClientConfig.builder()
                ...
                .clientName("testcontainers-extensions-valkey")
    ...
    private List<ValkeyValue> getValuesByKeys(@NotNull Collection<ValkeyKey> keys) {
        if (keys.isEmpty()) {
            return Collections.emptyList();
        }

        final byte[][] keysAsBytes = keys.stream()
                .map(ValkeyKey::asBytes)
                .toArray(byte[][]::new);
    ...
    private List<ValkeyValue> getValuesByPrefix(ValkeyKey keyPrefix) {
        try {
            final byte[] prefix = (keyPrefix.asString() + "*").getBytes(StandardCharsets.UTF_8);
            final List<ValkeyKey> keys = connection().keys(prefix).stream()
                    .filter(Objects::nonNull)
                    .map(ValkeyKey::of)
                    .collect(Collectors.toList());
    ...
    @Override
    public int countPrefix(@NotNull ValkeyKey keyPrefix) {
        return getValuesByPrefix(keyPrefix).size();
    }

    @Override
    public int count(@NotNull ValkeyKey... keys) {
        return count(List.of(keys));
    }

    @Override
    public int count(@NotNull Collection<ValkeyKey> keys) {
    ...
    @Override
    public void assertCountsPrefixNone(@NotNull ValkeyKey keyPrefix) {
        assertCountsPrefixEquals(0, keyPrefix);
    }

    @Override
    public void assertCountsNone(@NotNull ValkeyKey... keys) {
        assertCountsNone(Arrays.asList(keys));
    }

    @Override
    public void assertCountsNone(@NotNull Collection<ValkeyKey> keys) {
        final List<ValkeyValue> values = getValuesByKeys(keys);
    ...
    @Override
    public List<ValkeyValue> assertCountsPrefixAtLeast(long expectedAtLeast, @NotNull ValkeyKey keyPrefix) {
        final List<ValkeyValue> keyToValue = getValuesByPrefix(keyPrefix);
    ...
    @Override
    public List<ValkeyValue> assertCountsPrefixEquals(long expected, @NotNull ValkeyKey keyPrefix) {
        final List<ValkeyValue> keyToValue = getValuesByPrefix(keyPrefix);
        final long count = keyToValue.size();
    ...
    @Override
    public List<ValkeyValue> assertCountsAtLeast(long expectedAtLeast, @NotNull ValkeyKey... keys) {
        return assertCountsAtLeast(expectedAtLeast, List.of(keys));
    }

    @Override
    public List<ValkeyValue> assertCountsAtLeast(long expectedAtLeast, @NotNull Collection<ValkeyKey> keys) {
        final List<ValkeyValue> values = getValuesByKeys(keys);
        final long count = values.size();
        if (count < expectedAtLeast) {
            Assertions.assertEquals(expectedAtLeast, count,
    ...
    @Override
    public List<ValkeyValue> assertCountsEquals(long expected, @NotNull ValkeyKey... keys) {
        return assertCountsEquals(expected, List.of(keys));
    }

    @Override
    public List<ValkeyValue> assertCountsEquals(long expected, @NotNull Collection<ValkeyKey> keys) {
        final List<ValkeyValue> values = getValuesByKeys(keys);
        final long count = values.size();
    ...
    public boolean equals(Object o) {
        ...
        if (o == null || getClass() != o.getClass())
            return false;
        ValkeyConnectionImpl that = (ValkeyConnectionImpl) o;
